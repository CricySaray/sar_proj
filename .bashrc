...
# NOTE: 整段 bash-completion 框架已被注释，原因：
#   bash_completion 内部用 `. file` source 它的子文件 (/etc/bash_completion.d/*)，
#   而你定义 alias .='cd ..' 会劫持这个 `.`，导致 `cd: too many arguments`。
#   下面的 git 补全改为直接 source git 自己的补全文件，它是自包含的。
