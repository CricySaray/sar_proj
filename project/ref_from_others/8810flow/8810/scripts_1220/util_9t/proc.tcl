		...
		if { $type_name == "all" } {
			...
			foreach cell $vars(exclude_cells) {
				if { [dbGet head.allCells.name $cell] != "0x0" } {
					if {[lsearch $excludeCellInstList $cell] < 0 } {
						lappend excludeCellInstList $cell
					}
				}

			}
			saveNetlist -phys -excludeLeafCell -excludeCellInst "$excludeCellInstList" -excludeTopCellPGPort $vars(exlude_pg_list)  $dsn_gate_dir/[dbgDesignName].lvs.${view_name}.vg.gz
...
proc summarizeDesign {file} {
	...
	set bndy_pblk_box ""
	foreach box [dbGet [dbGet -p top.fplan.pBlkgs.name bundaryBLK].boxes ] {
