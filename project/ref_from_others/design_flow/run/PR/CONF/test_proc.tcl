...
proc importv {namespace {allow_overwrite 0} {verbose 0}} {
  ...
  if {[llength [info level 0]] < 1 || [llength [info level 0]] > 3} {
    error "proc importv: importv requires 1 to 3 arguments: namespace, [allow_overwrite=0], [verbose=0]"
  }
  
  if {$allow_overwrite ne "0" && $allow_overwrite ne "1"} {
    error "proc importv: allow_overwrite must be 0 or 1"
  }
  
  if {$verbose ne "0" && $verbose ne "1"} {
    error "proc importv: verbose must be 0 or 1"
  }
  ...
  foreach full_var $vars {
    ...
    if {![string is wordchar -strict $var_name] || [string match {[0-9]*} $var_name]} {
      error "proc importv: Invalid variable name in '$full_var': '$var_name' (must start with letter, alphanumerics only)"
    }
    
    # Check for duplicates in import set
    if {[info exists temp_import($var_name)]} {
      error "proc importv: Duplicate variable names in import: '$var_name' found in both '$temp_import($var_name)' and '$full_var'"
    }
    ...
    if {[info exists var_index($var_name)]} {
      if {!$allow_overwrite} {
        error "proc importv: Name conflict: '$var_name' already exists at '$var_index($var_name)' (cannot import from '$full_var'). Use allow_overwrite=1 to override."
      } else {
...
proc clear_namespace_vars {namespace} {
  ...
  # Check if namespace exists
  if {![namespace exists $namespace]} {
    error "proc clear_namespace_vars: Namespace '$namespace' does not exist"
  }
...
proc unsetv {var_name_or_path} {
  ...
  if {[string first "::" $var_name_or_path] != -1} {
    ...
    if {![info exists $var_name_or_path]} {
      error "proc unsetv: Variable path does not exist: $var_name_or_path"
    }
    set var_name [lindex [split $var_name_or_path "::"] end]
    if {![info exists var_index($var_name)] || $var_index($var_name) ne $var_name_or_path} {
      error "proc unsetv: Variable '$var_name_or_path' is not managed by this system"
    }
  } else {
    # Treat as simple name
    if {![info exists var_index($var_name_or_path)]} {
      error "proc unsetv: Variable '$var_name_or_path' not found in index"
    }
    set var_name $var_name_or_path
    set full_var $var_index($var_name)
    if {![info exists $full_var]} {
      error "proc unsetv: Variable path missing: $full_var. Index may be corrupted"
    }
  }
...
proc linkv {{args ""}} {
  ...
  if {[llength $args] > 0 && [lindex $args 0] eq "-force"} {
    if {[llength $args] < 2} {
      error "proc linkv: -force requires a value (0 or 1)"
    }
    set force [lindex $args 1]
    if {$force ne "0" && $force ne "1"} {
      error "proc linkv: -force must be 0 or 1"
    }
  ...
  if {[array size existing_vars] > 0 && !$force} {
    set msg "proc linkv: Cannot link variables - existing in current scope:\n"
