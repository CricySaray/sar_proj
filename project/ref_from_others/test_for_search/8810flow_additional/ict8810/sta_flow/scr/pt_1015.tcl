...
set search_path "$search_path $sh_launch_dir $sh_launch_dir/$sdir/scr"
...
foreach corner $dmsa_corners {
 foreach mode $dmsa_modes {
   foreach check $dmsa_check {	 
	if {[file exists /eda_files/proj/ict8810/backend/be8801/tmp/corner_sta/${corner}_${check}]} {
	  create_scenario \
	   -name ${mode}_${corner}_${check} \
	   -specific_variables {mode corner check VIEW current_dir RUN_ONECORNE  cdc_run  } \
