...
# Validate sources
unless (@sources) {
  die "No source paths specified. Use -s to specify files/directories. Use -h for help.\n";
}
...
sub print_help {
  print <<'HELP';
Usage: checksum_calculator.pl [options] -s <path1> [-s <path2> ...]
...
Options:
  -s, --source        Specify source file or directory path (required, multiple allowed)
...
Examples:
  1. Basic usage with directory statistics
     perl checksum_calculator.pl -s ./documents/
  
  2. Full format with directory and file counts
     perl checksum_calculator.pl -f full -s ./project/ -o results.txt
  
  3. Mixed sources with statistics
     perl checksum_calculator.pl -s ./data/ -s notes.txt -s reports/
