...
DEFAULT_OUTPUT_DIR = os.path.expanduser("~/downloaded_scripts")
CONFIG_FILE_PATH = os.path.expanduser("~/.script_distributor.conf")
...
USER_DEFINED_COMMANDS = [
  {
    ...
    "help": "Source environment configuration from env.sh in current directory"
  },
  {
    "alias": "run-flow",
    "command": "bash ./flow_scripts/run.sh",
    "help": "Execute flow script runner (requires flow_scripts/ directory)"
  },
  {
    "alias": "validate-perl",
    "command": "perl ./validation.pl --verbose",
    "help": "Run Perl validation script with verbose output"
  },
  {
    "alias": "check-deps",
    "command": "pip3 check --quiet || echo 'Missing Python dependencies'",
    "help": "Check for missing Python dependencies (silent success, verbose failure)"
  }
]
...
Examples:
  ...
  exec_parser = subparsers.add_parser(
    ...
    formatter_class=argparse.RawDescriptionHelpFormatter,
    epilog="""
Available Commands (alias -> command):
  {}\n
Examples:
  ...
  4. Silent execution (check dependencies):
     %(prog)s check-deps
    """.format(
      "\n  ".join([f"{cmd['alias']:<12} {cmd['command']}" for cmd in USER_DEFINED_COMMANDS])
    )
  )
