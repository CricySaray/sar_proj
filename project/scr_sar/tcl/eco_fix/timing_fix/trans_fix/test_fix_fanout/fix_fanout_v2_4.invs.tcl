...
proc analyzePowerDistribution {rootPoint leafPoints branchSegments generatorCapacity {optionsDict {}}} {
  ...
  dict set powerPlan repeaterDrivenLoads [dict create]  ;# Track loads driven by each repeater
...
proc decideAutomaticRepeaterUsage {avgDistance clusterSize totalLoads generatorCapacity loadCapacity usedRepeaters maxRepeaters} {
  # Calculate if using a repeater would be more efficient
  set clusterLoad [expr {$clusterSize * $loadCapacity}]
  set generatorRemaining [expr {$generatorCapacity - ($usedRepeaters * 4)}]  ;# Assume 4 units for repeater overhead
  
  # Use heuristic based on distance, load size, and remaining generator capacity
...
# Determine if a cluster should use a repeater (original logic, now used in specific strategies)
