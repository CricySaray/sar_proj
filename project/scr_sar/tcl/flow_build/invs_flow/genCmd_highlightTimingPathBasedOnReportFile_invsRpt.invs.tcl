...
#                   and then : set_global report_timing_format {hpin cell [load fanout pin_location user_derate socv_derate total_derate slew ... delay_mean delay_sigma incr_delay] delay arrival}
#             NOTICE: incr_delay: noise
#                     slew: transition
#                   and then : report_timing -net [-from ... -to ...] > file.rpt
# TODO      : U001: Specify the report_timing_format to facilitate obtaining the two values of delay and arrival time. Extract them and display them in the GUI.
# return    : cmds list
# ref       : link url
# --------------------------
...
proc genCmd_highlightTimingPathBasedOnReportFile_invsRpt {args} {
  ...
  set lineExpToSplitPath                  {^split} ; # can use regexp to split specific timing path, Has the same priority as startOfPath and endOfPath
...
proc genCmd_getPurePinOfPath_fromTimingPathReport_invsRpt {args} {
  ...
  set filteredPinCelltypeNetList [lmap tempList $splitedPinList {
    if {[llength $tempList] == 1} { continue } else {
      ...
      foreach {pin_celltype_net_1 pin_celltype_net_2} $tempList {
        ...
        set net2 [dbget [dbget top.insts.instTerms.name [lindex $pin_celltype_net_2 0] -p].net.name -e]
        if {$net1 == "" || $net2 == ""} {
          error "proc genCmd_getPurePinOfPath_fromTimingPathReport_invsRpt: check your pinname([lindex $pin_celltype_net_1 0] or [lindex $pin_celltype_net_2 0]) in rpt file($reportTimingFile), not found net!!! (in filteredPinCelltypeNetList)" 
        }
        if {$net1 != $net2} {
          error "proc genCmd_getPurePinOfPath_fromTimingPathReport_invsRpt: both pins([lindex $pin_celltype_net_1 0] and [lindex $pin_celltype_net_2 0]) are not on same net(net1: $net1 , net2: $net2)!!! (in filteredPinCelltypeNetList)"
        }
      }
    }
    set temp $tempList
  ...
  set j 0
  if {0} { ; # holdon
    foreach temp_sub_list $filteredPinCelltypeNetList {
  ...
  #return [list $filteredPinCelltypeNetList $instName_instBox2_3_incrCellDelay]
  return [list $filteredPinCelltypeNetList]
}
...
define_proc_arguments genCmd_getPurePinOfPath_fromTimingPathReport_invsRpt \
  ...
  -define_args {
    ...
    {-indexOfPureNumberOnLine "When the content in the 'line' becomes pure numbers (with non-numeric parts removed), the number of items in 'delay'" AString string optional}
    {-indexOfAfterSplitOriginalLineList "specify the index of list after spliting original lines" AInt int optional}
    {-lineExpToSplitPath "specify expression of spliting timing path, Has the same priority as startOfPath and endOfPath" AString string optional}
    {-startOfPath "specify the start expression of timing path" AString string optional}
    {-endOfPath "specify the end expression of timing path" AString string optional}
  }
