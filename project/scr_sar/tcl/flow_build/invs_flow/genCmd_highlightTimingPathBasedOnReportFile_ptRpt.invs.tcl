...
# update    : 2025/09/27 23:26:50 Saturday
#             (U001) Added a new feature: You can add your own starting points in the reportFile, or you can have no starting point identifier. If there is no identifier, 
#             all lines in the reportFile will be processed. If there is an identifier, the reportFile will first be truncated according to the identifier before processing. 
#             If there are multiple identifiers for the specified end lines, the lines after the last end line identifier will be removed. If there are multiple identifiers 
#             for the specified start lines, the lines before the first start line identifier will be removed, and only the remaining lines will be processed. This eliminates 
#             the need to additionally write the paths that need to be processed into another file; you can directly mark them in the complete reportFile and let the proc 
#             only process the paths within the marked range. This makes operations more convenient and reduces text copying and pasting.
#             related: $startLineExpOfReportTimingFile and $endLineExpOfReportTimingFile
# update    : 2025/09/27 23:33:07 Saturday
#             (U002) Added a new feature: The proc can now monitor the obtained incremental delay values and net length values. If the outer net length of a certain instance 
#             exceeds a specified threshold, it will be marked with a designated marker. Similarly, if the incremental delay value of an instance exceeds the specified 
...
alias ht "genCmd_highlightTimingPathBasedOnReportFile"
proc genCmd_highlightTimingPathBasedOnReportFile {args} {
  ...
  if {![is_human_readable_file $reportTimingFile]} {
    error "proc genCmd_getPurePinOfPath_fromTimingPathReport: invalid report file type(not human readable file): $reportTimingFile !!!" 
  }
...
proc genCmd_getPurePinOfPath_fromTimingPathReport {args} {
  ...
  if {[lsearch -regexp $content $startLineExp] != -1 || [lsearch -regexp $content $endLineExp] != -1} {
    set endIndex [lsearch -regexp $content $endLineExp]
    if {$endIndex != -1} {
