...
# usage     : 
# NOTICE    : please generate constraint report file using command: report_constraint -all_violator -path_type end|slack_only -nos -max_delay -min_delay -max_transition -max_capacitance -max_fanout -min_period -min_pulse_width -pba_path path|exhaustive
#             generate annotated report file using report_annotated_parasitics -check -pin_to_pin_nets
#             generate global timing csv file using report_global_timing -format csv -output outputfile.name
...
proc collect_sum_csv_file {{searchDir "./"} {formatOfScenarios "<mode>_<type>_<voltage>_<rcCorner>_<temperature>"} {modesOfFormatExp {func scan}} {typeToCheckOfFormatExp {setup hold}} {voltageOfFormatExp {0p99v 1p1v 1p21v}} {rcCornerOfFormatExp {cworst cbest rcworst rcbest typical}} {temperatureOfFormatExp {25c m40c 125c}}} {
  ...
  foreach temp_scenario $scenarioDirs {
    if {[file isfile "$temp_scenario/sum.csv"]} {
      ...
      lappend all_content [lindex $temp_content 0]
    } else {
      puts "Notice : dir: $temp_scenario have no sum.csv file!!!"
    }
  }
  set table_of_whole_sum_csv [print_formattedTable [concat [list $final_title] $all_content]]
...
proc runCmd_summarize_pt_rpts {args} {
  ...
  set infoOfAllScenarios [linsert $infoOfAllScenarios 0 [list scenario wns num tns r2r_w r2r_n r2r_t transW transN maxfanW maxfanN maxCapW maxCapN minPeriodW minPeriodN minPulseW minPulseN not_annotated]]
